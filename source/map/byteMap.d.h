	...
	#define eraseByteMapEndInPoint          ( 1 << 1 )
	...
	#define eraseByteMapMissingChildPoint   ( 1 << 3 )
		/* Отсутсвует необходимый дочерний узел, возвращаем "элемент не нейден" */
	#define eraseByteMapNotEqualPoint       ( 1 << 4 )
		/* Соответствующие байты аргумента и ноды различаются, возвращаем "элемент не нейден" */
	#define eraseByteMapNotModifyOwnerPoint ( 1 << 5 )
		/* Узел существует, на родительском узле есть значение или другие дочерние узлы(больше одного) */
	#define eraseByteMapNoValuePoint        ( 1 << 6 )
		/* Узел существует, но не имеет значения, возвращаем "элемент не нейден" */
	#define eraseByteMapSetValuePoint       ( 1 << 7 ) /* Заполнение результатов */
	#define eraseByteMapPointMask ( eraseByteMapEmpty | eraseByteMapEndIn | eraseByteMapLastNodePoint | \
		eraseByteMapMissingChild | eraseByteMapNotModifyOwnerPoint | eraseByteMapNotEqual | \
		eraseByteMapNoValue | eraseByteMapSetValuePoint )
